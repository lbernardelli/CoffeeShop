...
RSpec.describe CoffeeApp::OrderManager do
  ...
  describe 'credit_all' do
    ...
    context 'Process a list of payments' do
      ...
      it('Coach payed 25.49') { expect(subject.users[:coach].total_paid).to eql BigDecimal.new('25.49') }
      it('rochelle payed 7.75') { expect(subject.users[:rochelle].total_paid).to eql BigDecimal.new('7.75') }
      it('ellis payed 0.00') { expect(subject.users[:ellis].total_paid).to eql BigDecimal.new('0.00') }
    end
  end
